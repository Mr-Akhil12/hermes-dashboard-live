...
import {
  Pause, Play, Trash2, X, AlertTriangle, LayoutGrid, List,
} from 'lucide-react';
    ...
    try {
      ...
      for (const group of Object.values(bySchedule)) {
        for (let i = 0; i < group.length - 1; i++) {
          flowEdges.push({ id: `e${edgeIdx++}`, source: group[i].id, target: group[i + 1].id, style: { stroke: '#2a2a3a', strokeWidth: 1 }, markerEnd: { type: MarkerType.ArrowClosed, color: '#2a2a3a' } });
        }
      }
      setNodes(flowNodes);
      setEdges(flowEdges);
          ...
          <div className="flex items-center gap-1.5 md:gap-2 overflow-x-auto min-w-0">
            {(['all', 'active', 'paused', 'error'] as const).map(f => (
              <button key={f} onClick={() => setFilter(f)} className={`px-2 md:px-3 py-1 rounded-full text-[10px] md:text-xs font-medium transition-colors flex-shrink-0 ${filter === f ? 'bg-orange-500/10 text-orange-400' : 'text-zinc-500 hover:text-zinc-300'}`}>
                ...
                <span className="ml-1 text-[9px] md:text-[10px] text-zinc-600">({f === 'all' ? jobs.length : jobs.filter(j => f === 'active' ? j.enabled !== false && j.state !== 'paused' : f === 'paused' ? j.enabled === false || j.state === 'paused' : j.last_status === 'error').length})</span>
              </button>
            ))}
          </div>
        ...
        <div className="flex-1 overflow-auto">
          ...
          {view === 'graph' ? (
            <div className="h-full">
              <ReactFlow nodes={nodes} edges={edges} onNodesChange={onNodesChange} onEdgesChange={onEdgesChange} onNodeClick={(_, node) => { const job = jobs.find(j => j.id === node.id); if (job) setSelectedJob(job); }} fitView attributionPosition="bottom-left">
            ...
            <div className="p-3 md:p-5">
              <div className={`${theme.bgCard} rounded-xl overflow-x-auto`} style={{ border: '1px solid #1e1e2a' }}>
                <table className="w-full text-xs min-w-[600px]">
                  <thead><tr className="text-zinc-500 uppercase tracking-wider" style={{ borderBottom: '1px solid #1e1e2a' }}>
                    <th className="text-left px-3 md:px-4 py-3 font-medium">Job</th><th className="text-left px-3 md:px-4 py-3 font-medium">Schedule</th><th className="text-left px-3 md:px-4 py-3 font-medium">Status</th><th className="text-left px-3 md:px-4 py-3 font-medium hidden sm:table-cell">Last Run</th><th className="text-left px-3 md:px-4 py-3 font-medium hidden md:table-cell">Delivery</th><th className="text-right px-3 md:px-4 py-3 font-medium">Actions</th>
                  </tr></thead>
                  <tbody>
                    {jobs.filter(j => { if (filter === 'active') return j.enabled !== false && j.state !== 'paused'; if (filter === 'paused') return j.enabled === false || j.state === 'paused'; if (filter === 'error') return j.last_status === 'error'; return true; }).map(job => {
                      const status = job.last_status === 'error' ? 'error' : job.state === 'paused' || job.enabled === false ? 'paused' : 'ok';
                      ...
                      return (
                        <tr key={job.id} className="hover:bg-zinc-800/30 cursor-pointer transition-colors" style={{ borderBottom: '1px solid #16161f' }} onClick={() => setSelectedJob(job)}>
                          ...
                          <td className="px-3 md:px-4 py-2.5 md:py-3"><span className={`inline-flex items-center gap-1 px-1.5 md:px-2 py-0.5 rounded-full text-[9px] md:text-[10px] font-medium ${sc.bg} ${sc.text}`}><div className={`w-1 h-1 rounded-full ${sc.dot}`} /><span className="hidden xs:inline">{status}</span></span></td>
                          ...
                          <td className="px-3 md:px-4 py-2.5 md:py-3 text-right">
                            <button onClick={(e) => { e.stopPropagation(); handlePause(job.id); }} className="p-1 rounded hover:bg-zinc-800 text-zinc-500 hover:text-zinc-300" title={job.enabled === false || job.state === 'paused' ? 'Resume' : 'Pause'}>{(job.enabled === false || job.state === 'paused') ? <Play className="w-3 h-3" /> : <Pause className="w-3 h-3" />}</button>
                            <button onClick={(e) => { e.stopPropagation(); setConfirmDelete(job.id); }} className="p-1 rounded hover:bg-zinc-800 text-zinc-500 hover:text-red-400 ml-1" title="Delete"><Trash2 className="w-3 h-3" /></button>
                          </td>
                        </tr>
                      );
                    })}
      ...
      {selectedJob && (
        <div className="w-80 bg-[#0d0d14] overflow-y-auto flex-shrink-0 hidden md:flex flex-col" style={{ borderLeft: '1px solid #1e1e2a' }}>
          ...
          <div className="p-4 space-y-4 flex-1 overflow-y-auto">
            ...
            <div><p className="text-[10px] text-zinc-600 uppercase tracking-wider">Status</p>{(() => { const s = selectedJob.last_status === 'error' ? 'error' : selectedJob.state === 'paused' || selectedJob.enabled === false ? 'paused' : 'ok'; const sc = getStatusColor(s); return (<span className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-xs font-medium mt-1 ${sc.bg} ${sc.text}`}><div className={`w-1.5 h-1.5 rounded-full ${sc.dot}`} />{s}</span>); })()}</div>
            ...
            <div className="bg-orange-500/5 border border-orange-500/10 rounded-lg p-3"><p className="text-[11px] text-orange-400 font-medium mb-1">Editing</p><p className="text-[10px] text-zinc-500">To edit, DM Hermes on Discord:<br /><span className="text-zinc-400 font-mono">{selectedJob.name} — [change]</span></p></div>
            <div className="flex gap-2">
              <button onClick={() => handlePause(selectedJob.id)} className={`flex-1 flex items-center justify-center gap-2 px-3 py-2 rounded-lg text-xs font-medium ${(selectedJob.enabled === false || selectedJob.state === 'paused') ? 'bg-emerald-500/10 text-emerald-400' : 'bg-amber-500/10 text-amber-400'}`}>{(selectedJob.enabled === false || selectedJob.state === 'paused') ? <Play className="w-3 h-3" /> : <Pause className="w-3 h-3" />}{(selectedJob.enabled === false || selectedJob.state === 'paused') ? 'Resume' : 'Pause'}</button>
              <button onClick={() => setConfirmDelete(selectedJob.id)} className="flex items-center justify-center gap-2 px-3 py-2 rounded-lg text-xs font-medium bg-red-500/10 text-red-400"><Trash2 className="w-3 h-3" />Delete</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-x-0 bottom-0 z-50 bg-[#0d0d14] rounded-t-2xl max-h-[70vh] overflow-y-auto md:hidden" style={{ borderTop: '1px solid #1e1e2a' }}>
          ...
          <div className="p-4">
            <div className="grid grid-cols-2 gap-3 mb-3">
              ...
              <div><p className="text-[9px] text-zinc-600 uppercase tracking-wider">Status</p>{(() => { const s = selectedJob.last_status === 'error' ? 'error' : selectedJob.state === 'paused' || selectedJob.enabled === false ? 'paused' : 'ok'; const sc = getStatusColor(s); return (<span className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[10px] font-medium mt-0.5 ${sc.bg} ${sc.text}`}><div className={`w-1 h-1 rounded-full ${sc.dot}`} />{s}</span>); })()}</div>
            ...
            <div className="flex gap-2">
              <button onClick={() => handlePause(selectedJob.id)} className={`flex-1 flex items-center justify-center gap-2 px-3 py-2.5 rounded-lg text-xs font-medium ${(selectedJob.enabled === false || selectedJob.state === 'paused') ? 'bg-emerald-500/10 text-emerald-400' : 'bg-amber-500/10 text-amber-400'}`}>{(selectedJob.enabled === false || selectedJob.state === 'paused') ? <Play className="w-3 h-3" /> : <Pause className="w-3 h-3" />}{(selectedJob.enabled === false || selectedJob.state === 'paused') ? 'Resume' : 'Pause'}</button>
              <button onClick={() => setConfirmDelete(selectedJob.id)} className="flex items-center justify-center gap-2 px-3 py-2.5 rounded-lg text-xs font-medium bg-red-500/10 text-red-400"><Trash2 className="w-3 h-3" />Delete</button>
            </div>
          </div>
        </div>
      )}
