...
export default function CronPage() {
  ...
  // On mobile, force table view (React Flow doesn't work well on small screens)
    ...
    <div className={`flex h-[calc(100vh-3.5rem)] ${theme.bg}`} style={{ marginLeft, transition: 'margin-left 0.3s' }}>
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-w-0">
          ...
          <div className="flex items-center gap-1.5 md:gap-2 overflow-x-auto min-w-0">
            {(['all', 'active', 'paused', 'error'] as const).map(f => (
              <button
                key={f}
                onClick={() => setFilter(f)}
                className={`px-2 md:px-3 py-1 rounded-full text-[10px] md:text-xs font-medium transition-colors flex-shrink-0 ${
        ...
        <div className="flex-1 overflow-auto">
          {error && (
            <div className="mx-3 md:mx-5 mt-3 bg-red-500/10 border border-red-500/20 rounded-lg px-3 py-2 text-xs text-red-400">
              {error}
            </div>
          )}
      ...
      {/* Side panel — hide on mobile, use bottom sheet instead */}
      {selectedJob && !isMobile && (
        <div className="w-80 bg-[#0d0d14] overflow-y-auto flex-shrink-0" style={{ borderLeft: '1px solid #1e1e2a' }}>
          ...
          <div className="p-4 space-y-4">
            ...
            <div>
              <p className="text-[10px] text-zinc-600 uppercase tracking-wider">Delivery</p>
      ...
      {/* Mobile bottom sheet for selected job */}
      {selectedJob && isMobile && (
        <div className="fixed inset-x-0 bottom-0 z-50 bg-[#0d0d14] rounded-t-2xl max-h-[70vh] overflow-y-auto" style={{ borderTop: '1px solid #1e1e2a' }}>
          <div className="flex items-center justify-between px-4 py-3" style={{ borderBottom: '1px solid #1e1e2a' }}>
            <h3 className="text-sm font-semibold text-zinc-200 truncate">{selectedJob.name}</h3>
            <button onClick={() => setSelectedJob(null)} className="p-1 text-zinc-500">
              <X className="w-5 h-5" />
            </button>
          </div>
          <div className="p-4 space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <div>
                <p className="text-[9px] text-zinc-600 uppercase tracking-wider">Schedule</p>
                <p className="text-xs text-zinc-200 mt-0.5">{selectedJob.schedule_display || '—'}</p>
              </div>
              <div>
                ...
                {(() => {
                  ...
                  const sc = getStatusColor(s);
                  return (
                    <span className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[10px] font-medium mt-0.5 ${sc.bg} ${sc.text}`}>
                      <div className={`w-1 h-1 rounded-full ${sc.dot}`} />
                      {s}
                    </span>
                  );
              ...
              <div>
                <p className="text-[9px] text-zinc-600 uppercase tracking-wider">Last Run</p>
                <p className="text-xs text-zinc-200 mt-0.5">{selectedJob.last_run_at ? new Date(selectedJob.last_run_at).toLocaleDateString('en-ZA', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' }) : '—'}</p>
              </div>
              <div>
                <p className="text-[9px] text-zinc-600 uppercase tracking-wider">Delivery</p>
                <p className="text-xs text-zinc-200 mt-0.5 font-mono truncate">{selectedJob.deliver || '—'}</p>
              </div>
            </div>
            <div className="flex gap-2">
              <button
                onClick={() => handlePause(selectedJob.id)}
                className={`flex-1 flex items-center justify-center gap-2 px-3 py-2.5 rounded-lg text-xs font-medium ${
                  selectedJob.enabled === false || selectedJob.state === 'paused'
              ...
              <button
                onClick={() => setConfirmDelete(selectedJob.id)}
                className="flex items-center justify-center gap-2 px-3 py-2.5 rounded-lg text-xs font-medium bg-red-500/10 text-red-400"
              >
                <Trash2 className="w-3 h-3" />
                Delete
              </button>
